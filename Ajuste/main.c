...
void textbackground (int BckgC)
{
    ...
    if (GetConsoleScreenBufferInfo (hStdOut, &csbi))
    {
        wColor = (csbi.wAttributes & 0x0F) + (BckgC & 0xF0);
        SetConsoleTextAttribute (hStdOut, wColor);
    }
...
void tela_inicio (char what)
{
    ...
    char c = 1;
    gotoxy (1, 7);
    _cprintf ("Entre com a fun‡„o g%d de vari vel ' x '(g0(x), g1(x), ...) :", what);
    gotoxy (1, 9);
    putch ('[');
    gotoxy (64, 9);
    putch (']');
...
void push (struct REGTOKEN *token)
{
    ...
    {
        _cprintf (MSGERROPILHA);
        erro = VERDADE;
    }
    else
        pilha[++topo_pilha] = *token;
}
///////////////////////////////////////////////////////////////////////
struct REGTOKEN pop (void)
{
...
void reduza (int reducao)
{
    ...
    switch (reducao)
    {
        ...
        case 5 :
            ...
            if (fabs (token1.x.valor) == 0)
                token_atual.x.valor = HUGE_VAL;
            else
                token_atual.x.valor = token2.x.valor / token1.x.valor;
        ...
        case 11 :
            ...
            if (fabs (token1.x.valor) == 0)
                ...
                token_atual.x.valor = (double) ( (long) token2.x.valor % (long) token1.x.valor);
...
void Mostra_Funcao (char x, char y, char *funcao)
{
    ...
    {
        if ( (funcao[cont] != 65) && (funcao[cont + 1] != 49))
            putch (funcao[cont]);
