...
void main(void)
{
    ...
    getimage(x-4,y-3,x+4,y+3,alvo);
	 xmax = 316;      xmin = 4;
	 ymax = 198;      ymin = 2;
	 while (kbhit() != 0) c=getch();   /* Zera buffer do teclado */
	 while (kbhit()==0)
	 {
...
comecar:
		...
		if (disparo==1)
	{
      ...
      delay(10);
		putimage(x-4,y-3,alvo,COPY_PUT);
		x=x+q;  y=y+w;
		if ((x==xmax-3)|(x==xmin+3)|
	  (getpixel(x+4,y)==3)|(getpixel(x-4,y)==3)) q=q*(-1);
		if ((y==ymax-4)|(y==ymin+4)|
	  (getpixel(x,y+3)==3)|(getpixel(x,y-3)==3)) w=w*(-1);
		if ((t <= ymin)|(z <= xmin)|(z >= xmax))
	  {
		 ...
		 setcolor(0);
		 circle(z,t,1);
		 z=(160+(tiro*cos(angulo*3.1415926 / 180)));
		 t=(189+(-tiro*sin(angulo*3.1415926 / 180))/1.1);
		 setcolor(3);
		 rectangle(2,2,318,198);
		};
		if (getpixel(z,t)==2) c=27;
	 };
