...
int main()
{
  ...
  if( GraphDriver==EGA || GraphDriver==EGALO || GraphDriver==VGA || GraphDriver==134)
    PaletteDemo();
  PutPixelDemo();
  PutImageDemo();
  Bar3DDemo();
...
void Initialize(void)
{
  ...
  GraphDriver = installuserdriver("SVGA256", detectEGA);
...
/* Plot some "stars"  */
  for ( i=0 ; i<1000; ++i )
    putpixel(random(MaxX), random(MaxY), random( 15 )+1);
  x = MaxX / 2;
  y = MaxY / 2;
  PauseTime = 70;
...
void FillStyleDemo(void)
{
  ...
  for( j=0 ; j<3 ; ++j ){		/* Three rows of boxes		*/
    for( i=0 ; i<4 ; ++i ){		/* Four column of boxes 	*/
      setfillstyle(style, 15); /* Set the fill style and WHITE */
...
void FillPatternDemo(void)
{
  ...
  for( j=0 ; j<3 ; ++j ){		/* Three rows of boxes		*/
    for( i=0 ; i<4 ; ++i ){		/* Four column of boxes 	*/
      setfillpattern( &patterns[style][0], 15 );
      bar( x, y, x+w, y+h );		/* Draw the actual box		*/
      rectangle( x, y, x+w, y+h );	/* Outline the box		*/
      itoa( style, buffer, 10 );	/* Convert style 3 to ASCII	*/
      outtextxy( x+(w / 2), y+h+4, buffer );
...
void PaletteDemo(void)
{
  ...
  while( !kbhit() ){			/* Until user enters a key...	*/
    setrgbpalette(1+random(MaxColors-2),random(65),random(65),random(65));
  }
