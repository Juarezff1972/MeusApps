...
void main(void)
{
   ...
   sx=0;sy=24;coords();sx1=sx;sy1=sy;
	...
	{
	cor=1;
	if ((ch=='j')||(ch=='k')||(ch=='m')||(ch=='i')||
		 (ch=='[')||(ch==']')||(ch=='e')||(ch=='z')||
		 (ch=='w')||(ch=='x')||(ch=='a')||(ch=='d')) cor=0;
	do
	{
	p1=0;
   rotation();
   for (t2=1;t2<=6;t2++)
   {
	...
	moveZ=0;
	cor++;
	if (ch=='j') rotacaoX=-0.05;
	if (ch=='k') rotacaoX=+0.05;
	if (ch=='m') rotacaoY=-0.05;
	if (ch=='i') rotacaoY=+0.05;
	if (ch==']') vira=-0.05;
	if (ch=='[') vira=+0.05;
	if (ch=='a') moveX=+1;
	if (ch=='d') moveX=-1;
	...
	if (ch=='z') moveZ=-1;
	ch=0;
	if (r1>(2*pi)) r1=r1-(2*pi);
	if (r2>(2*pi)) r2=r2-(2*pi);
	if (r3>(2*pi)) r3=r3-(2*pi);
	r1=r1+rotacaoX;
	r2=r2+vira;
	r3=r3+rotacaoY;
	mx=mx+moveX;
...
{
	x=-x;
	xa=cr1*x-sr1*z;	za=sr1*x+cr1*z;	x=cr2*xa+sr2*y;
	ya=cr2*y-sr2*xa;	z=cr3*za-sr3*ya;	y=sr3*za+cr3*ya;
	x=x+mx;	y=y+my;	z=z+mz;	sx=d*x/z;	sy=d*y/z;
   return;
}
/*************************************************************************/
...
void viewport(void)
{
   ...
   if (sya>maxy) if (syb>maxx) return;
	if ((sxa<minx)&&(sxb!=sxa))
   {
      {
			c=(syb-sya)/(sxb-sxa)*(sxb-minx);sxa=minx;sya=syb-c;
      };
      if (sya<miny) if (syb<miny) return;
   ...
   {
      {
			c=(syb-sya)/(sxb-sxa)*(maxx-sxa);sxb=maxx;syb=sya-c;
      };
      if (sya<miny) if (syb<miny) return;
      if (sya>maxy) if (syb>maxy) return;
   };
...
void graphics_setup(void)
{
   ...
   ga=VGA;
	...
	initgraph(&ga,&gm,"");
	x_res=640;y_res=480;
	maxx=838;minx=1;maxy=678;miny=1;screen_x=639;screen_y=479;
	setpalette(0,8);
	setcolor(c12);
	outtextxy(0,472,"(C)1993 By JFF Software Ltd.");
	setcolor(c10);
	outtextxy(290,0,"Cubo 3D");
   return;
};
/*************************************************************************/
